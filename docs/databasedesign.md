...
- `class_subjects(class_id, subject_id)` — bảng junction thay cho `classes.subject_id` (1 lớp chỉ 1 môn trước đây), theo đúng khuôn P2 (junction table thay vì mảng ID) đã dùng cho `skill_node_prerequisites`. Sau khi backfill dữ liệu cũ, `classes.subject_id` bị **drop hẳn** — không giữ hai nguồn sự thật.
...
## Phần 9 — Bổ sung: Chọn lớp cụ thể 1-12 khi tạo lớp (migration 0006)

SQL đầy đủ ở `database/migrations/0006_class_exact_grade.sql`. Thực hiện đúng theo dõi ở Phần 7: `classes.grade_band` chỉ có 3 mức nên trước đây một lớp gắn `primary` có thể chọn bất kỳ môn tiểu học nào, kể cả "Khoa học" (chỉ dạy lớp 4-5) cho một lớp thực chất là lớp 1.

- `classes.grade` — smallint, tham chiếu `grades(grade_number)`, CHECK phải nằm trong đúng phạm vi của `grade_band` (1-5/6-9/10-12). Nullable vì lớp tạo trước migration này không có cách suy ra chính xác từ band — chỉ ép buộc ở tầng service cho lớp tạo mới.
- `validateClassSubjects` trong `classroomService.js` giờ so khớp `grade` với `subjects.min_grade`/`max_grade` của từng môn (rơi về so `grade_band` nếu môn chưa có phạm vi lớp, tức dữ liệu seed trước migration 0004) — giáo viên dạy lớp 1 sẽ không còn thấy/chọn được môn "Khoa học" trong danh sách.
- Frontend (`TeacherClassesPage.jsx`): sau khi chọn Khối lớp, giáo viên phải chọn thêm Lớp cụ thể (dropdown theo đúng khoảng của khối) trước khi danh sách môn học hiện ra.

Phụ thuộc thứ tự: migration `0006` cần `0004` (bảng `grades`, `subjects.min_grade`/`max_grade`) đã được apply trước, vì có FK tới `grades` và validate dựa trên `subjects.min_grade`/`max_grade`.
